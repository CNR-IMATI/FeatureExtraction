function [curveId, curvePars, axis1, axis2, indPts] = extract_circumf(modelName, nameFct, parN, minPts, eps, facet, erosion)
...
%             
    ...
    for k=1:s
        ...
        command = './CoCoA-5/cocoa5 recognitionAlgorithm_circumf.cocoa5';
...
%         hold on
        ...
        dist = xy(:,1).^2+xy(:,2).^2-a^2;
        outCross = find(dist<eps);
        outNotCross = find(dist>eps);
%         plot(xy(outCross,1),xy(outCross,2),'*k')
%         plot(xy(outNotCross,1),xy(outNotCross,2),'*r')
%         h = ezplot(@(x,y)circumf(x,y,a), [min(xy(:,1)), max(xy(:,1)), min(xy(:,2)), max(xy(:,2))])
