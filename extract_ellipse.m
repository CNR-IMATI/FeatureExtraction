function [curveId, curvePars, axis1, axis2, indPts] = extract_ellipse(modelName, nameFct, parN, minPts, eps, facet, erosion)
  
    % Perform the whole extraction pipeline using ellipse curve
    % 
    % Input:    
...
%             
            ...
            PtMin = [0.85*A 0.85*C];
            PtMax = [1.15*A 1.15*C];
            Eps = [(1/20)*(PtMax(1)-PtMin(1))  (1/20)*(PtMax(2)-PtMin(2))];
...
%         stringa1 = strcat('Source "/Users/torrente/work/Ricerche/CNR-IMATI/GRAVITATE/Dataset/algoritmi_Estrazione_Occhi/clusterPoints_', int2str(i), '.cocoa5";');
%         stringa2 = strcat('Source "/Users/torrente/work/Ricerche/CNR-IMATI/GRAVITATE/Dataset/algoritmi_Estrazione_Occhi/parameters_', int2str(i), '.cocoa5";');
        fprintf(fileID, '%s\n', stringa1);
        fprintf(fileID, '%s\n', stringa2);
        fclose(fileID);
       
        % run Hough transform (recognitionAlgorithm) on file
        command = './CoCoA-5/cocoa5 recognitionAlgorithm_ellipse.cocoa5';
...
%         h = ezplot(@(x,y)ellipse(x,y,a,c), [min(xy(:,1)), max(xy(:,1)), min(xy(:,2)), max(xy(:,2))])
...
function z = ellipse(x,y,a,c)
    z = c^2*x^2 + a^2*y^2-a^2*c^2;
end
