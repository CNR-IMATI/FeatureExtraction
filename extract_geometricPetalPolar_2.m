function [curveId, curvePars, axis1, axis2, indPts] = extract_geometricPetalPolar_2(modelName, nameFct, D, parN, minPts, eps, facet, erosion)
    ...
    %  'D'          is the degree of 'eccentricity' of the geometric petal curve (default=40)
...
%     title(['DBSCAN Clustering (\epsilon = ' num2str(epsilon) ', minPts = ' num2str(minPts) ')']);
    ...
    %%N = 40; % degree of geometric petal curve
...
%             end
            ...
            %
            costhetaRho = zeros(size(xy2new,1),2);
            for k=1:size(xy2new,1)
                rho = sqrt(xy2new(k,1)^2+xy2new(k,2)^2);
                costhetaRho(k,2) = rho;
%                 if (xy4(k,1)==0) 
%                     if (xy4(k,2)>0) theta = pi/2; else theta = (3./2)*pi; end;
%                 else
                theta = atan(xy2new(k,2)/xy2new(k,1));
                    %if (xy4(k,1)>0 & (xy4(k,2)<0)) theta=theta+2*pi; end;
                    if (xy2new(k,1)<0) theta = theta+pi;end;
                %end;
                costhetaRho(k,1) = cos(theta);
            end;
            ...
            fileID = fopen(nameFile,'w');
            fprintf(fileID, '%9s\n','Points:=[');
            fprintf(fileID,'[%14.12f, %14.12f], \n', costhetaRho(1:(size(xy2new,1)-1),:)');
            fprintf(fileID,'[%14.12f, %14.12f] \n', costhetaRho(size(xy2new,1),:)');
            fprintf(fileID, '%3s\n','];');
            fclose(fileID);
        end
    end
    ...
    for k=1:s
        ...
        command = './CoCoA-5/cocoa5 recognitionAlgorithm_geometricPetalPolar.cocoa5';
    ...
    while (Val(kk,1)>0 & kk>0)
        ...
        for j=0:500
            theta=(j/500)*pi;
            rho=a*(1-(cos(theta))^(2*D));
            xx = rho*cos(theta);
            yy = rho*sqrt(1-(cos(theta))^2);
            puntiCurva(j+1,1)=(1/c)*xx;
            puntiCurva(j+1,2)=yy;
        end
%         figure
%         axis equal
...
%         fclose(fileID);
        ...
        % salient points
        valueX = ((2*D)/(2*D+1))*(a/c)*nthroot(1/(2*D+1),2*D);
        valueY = ((2*D)/(2*D+1))*a*sqrt(1-nthroot(1/(2*D+1),2*D));
        salientPts=[-valueX valueY; valueX valueY; 0 0; 0 a];
...
function z = geometricPetal(x,y,D,a,c)
    z = (c*x^2+y^2)^(2*D+1) - a*((c*x^2+y^2)^D - c^D*x^(2*D))^2;
end
