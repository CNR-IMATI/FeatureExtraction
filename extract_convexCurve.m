%%% aggiungere parametro per usare geometria o colore?  ---> ora colore
function [curveId, curvePars, axis1, axis2, indPts] = extract_convexCurve(modelName, nameFct, D, parN, minPts, eps, facet, erosion)

    % Perform the whole extraction pipeline using Geometric Petal curve in polar form
    % 
    % Input:    
    ...
    % transform the ply file to an off file
    %filename = 'model.off';
    %write_off(filename, vertices, faces);
    
    % read the color
    ptCloud = pcread(modelName);
    CC = double(ptCloud.Color);
    
    %transform the color from RGB to CIELab
    [L,a,b] = RGB2Lab(CC(:,1), CC(:,2), CC(:,3));

    %histogram L
    hL = histogram(L, 'visible', 'off');
    set(gcf,'Visible','off');
    numBins = hL.NumBins;
    i=0;
    Values = hL.Values;
    ...
    % select feature points with low (smaller than the theshold) mean curvature or
    % luminosity L
...
%             end
             ...
             %riallineo
            MMx{1,i} = SP(4,1);
            MMy{1,i} = (SP(4,2)+SP(2,2))/2;
            xy2new(:,1)=xy2new(:,1)-MMx{1,i};
            xy2new(:,2)=xy2new(:,2)-MMy{1,i};
            SP(:,1) = SP(:,1)-MMx{1,i};
            SP(:,2) = SP(:,2)-MMy{1,i};
             
            % stretch and transform to polar coordinates
            maxX = max(abs(xy2new(:,1)));
            maxY = max(abs(xy2new(:,2)));
            if maxX>maxY
               SCx{1,i} = maxY/maxX;
               SCy{1,i} = 1;
               xy2new(:,1) = (maxY/maxX)*xy2new(:,1); 
            else
               SCx{1,i} = 1;
               SCy{1,i} = maxX/maxY;
               xy2new(:,2) = (maxX/maxY)*xy2new(:,2);
            end
            
            
            cosMthetaRho = zeros(size(xy2new,1),2);
            for k=1:size(xy2new,1)
                rho = sqrt(xy2new(k,1)^2+xy2new(k,2)^2);
                cosMthetaRho(k,2) = rho;
                if (xy2new(k,1)==0) 
                    if (xy2new(k,2)>0) theta = pi/2; else theta = (3./2)*pi; end;
                else
                    theta = atan(xy2new(k,2)/xy2new(k,1));
                    if (xy2new(k,1)>0 & (xy2new(k,2)<0)) theta=theta+2*pi; end;
                    if (xy2new(k,1)<0) theta = theta+pi;end;
                end;
                cosMthetaRho(k,1) = cos(D*theta);
            end;
            
            
            % give an estimate of the parameters 
            %%R1 = min(cosMthetaRho(:,2)); 
            R2 = max(cosMthetaRho(:,2));
            R1 = 0.9*R2;
            A = (2*R1*R2)/(R1+R2);
            B = (R2-R1)/(R1+R2);
            PtMin = [0.75*A, 0.75*B]; 
            PtMax = [1.25*A, 1.25*B];
            Eps = (1/5)*[PtMax(1)-PtMin(1) PtMax(2)-PtMin(2)];     
            
            % write parameters limits and epsilon on a file
            nameFile = strcat('parameters_', int2str(i) , '.cocoa5');
            fileID = fopen(nameFile,'w');
            fprintf(fileID, '%9s\n','PtMin:=');
            fprintf(fileID,'[%8.6f, %8.6f] \n', PtMin);
            fprintf(fileID, '%3s\n',';');
            fprintf(fileID, '%9s\n','PtMax:=');
            fprintf(fileID,'[%8.6f, %8.6f] \n', PtMax);
            fprintf(fileID, '%3s\n',';');
            ...
            fileID = fopen(nameFile,'w');
            fprintf(fileID, '%9s\n','Points:=[');
            fprintf(fileID,'[%14.12f, %14.12f], \n', cosMthetaRho(1:(size(xy2new,1)-1),:)');
            fprintf(fileID,'[%14.12f, %14.12f] \n', cosMthetaRho(size(xy2new,1),:)');
            fprintf(fileID, '%3s\n','];');
            fclose(fileID);
        end
    end
    ...
    for k=1:s
        ...
        command = './CoCoA-5/cocoa5 recognitionAlgorithm_convexCurve.cocoa5';
    ...
    % plot tutti i risultati - punti del cluster scelto con convex curve
    curvePars = [];
    kk=numCluster;
...
%         plot(xy(:,1),xy(:,2),'*m')
        ...
        puntiCurva = zeros(1001,2);
        for j=0:1000
            theta=(j/1000)*2*pi;
            rho=a/(1+b*cos(D*theta)); 
            xx = rho*cos(theta);
            yy = rho*sin(theta);
            puntiCurva(j+1,1)=xx;
            puntiCurva(j+1,2)=yy;
...
%         %h = ezplot(@(x,y)convexCurve(x,y,N,a^2,c^2), [min(xy(:,1)), max(xy(:,1)), 0, max(xy(:,2))])
        %set(h, 'Color', 'g','LineWidth',2)
%         plot(puntiCurva(:,1), puntiCurva(:,2),'.g');
%         title([])
...
% function z = convexCurve(x,y,M,a,b)
%     z = (c*x^2+y^2)^(2*N+1) - a*((c*x^2+y^2)^N - c^N*x^(2*N))^2;
% end
%  
